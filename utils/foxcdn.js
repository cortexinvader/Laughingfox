...
class foxcdnClient {
    ...
    connect(apiKey, deployedUrl) {
        if (!apiKey || !deployedUrl) {
            throw new Error(
                "API Key and deployed URL are required for connection."
            );
        }
        this.apiKey = apiKey;
    ...
    _checkInitialized() {
        if (!this.isInitialized) {
            throw new Error(
                "foxcdn Client is not initialized. Call foxcdn.connect(apiKey, url) first."
            );
        }
    }
    ...
    async _apiCall(endpoint, method = "GET", data = null) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                errorMessage =
                    typeof errorBody === "object" ? errorBody.error : errorBody;
    ...
    async upload(filePath) {
        ...
        try {
            ...
            if (error.code === "ENOENT") {
                throw new Error(
                    `Upload failed: File not found at path ${filePath}`
                );
            }
            throw error;
        }
