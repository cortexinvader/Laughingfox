  ...
  async mainFunc({ senderID, threadID, event, message, args, bot }) {
    try {
      if (
        ...
        !global.client.config.admins.includes(senderID.replace("@lid", ""))
      ) {
  ...
  async handleMessage(event) {
    try {
      ...
      if (
        ...
        !global.client.config.whitelist.ids.includes(senderID.split("@")[0])
      )
        return;
      ...
      const message = {
        ...
        sendGif: async (filepath, cap) => {
          return await this.sock.sendMessage(threadID, {
            video: { url: filepath, caption: cap || "", gifPlayback: true },
          });
        },
